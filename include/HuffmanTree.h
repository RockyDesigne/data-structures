//
// Created by HORIA on 16.01.2024.
//
#include "My_bt.h"
#ifndef DATA_STRUCTURES_HUFFMANTREE_H
#define DATA_STRUCTURES_HUFFMANTREE_H
...
private:
    ...
    static Node* huffman(const std::vector<Node*>& v) {
        auto comp {[](const Node* a, const Node* b){return a->frecv > b->frecv;}};
    ...
    std::string decode(Node* root, const std::string& code) {
        int i {};
        std::string decoded;
        while (i < code.size()) {
            Node* it {root};
...
#endif //DATA_STRUCTURES_HUFFMANTREE_H
